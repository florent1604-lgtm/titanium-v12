...
PATCH v12.1 :
- Client HTTP persistent avec keep-alive
- Timeout augmenté à 10s + retry avec backoff
- Health check proactif au démarrage
- Imports dynamiques mis en cache
- Distinction error 5xx vs timeout vs offline
"""
...
_HTTP_TIMEOUT = 10.0           # ↑ 4s → 10s (modèles ML lourds)
_HTTP_MAX_RETRIES = 2          # Nouveau : retry sur erreur réseau
_HTTP_RETRY_BACKOFF = 1.0

# ── Client HTTP persistent ──────────────────────────────────────────────────
# Un seul client réutilisé = connexions TCP keep-alive, pas de fuite
...
def _get_client() -> httpx.AsyncClient:
    """Retourne le client HTTP global, le crée si nécessaire."""
    global _http_client
...
async def _close_client() -> None:
    """Ferme proprement le client HTTP (appeler au shutdown)."""
    global _http_client
...
async def _get(path: str) -> dict[str, Any]:
    """Fetch Titanium REST avec retry et classification d'erreur."""
    client = _get_client()
    ...
    for attempt in range(1, _HTTP_MAX_RETRIES + 1):
        try:
            ...
            if 500 <= r.status_code < 600:
                # Erreur serveur = peut être temporaire, on retry
            ...
            if attempt < _HTTP_MAX_RETRIES:
                await asyncio.sleep(_HTTP_RETRY_BACKOFF * attempt)
                continue
            return {"status": "timeout", "reason": f"Request timeout after {_HTTP_TIMEOUT}s", "url": url}
            ...
            return {"status": "offline", "reason": f"{type(e).__name__}: {e}", "url": url}

    return {"status": "offline", "reason": "Max retries exceeded", "url": url}
...
async def _post(path: str, data: dict | None = None, headers: dict | None = None) -> dict[str, Any]:
    ...
    for attempt in range(1, _HTTP_MAX_RETRIES + 1):
        try:
            ...
            if r.status_code == 200:
                return r.json()
            if 500 <= r.status_code < 600 and attempt < _HTTP_MAX_RETRIES:
                await asyncio.sleep(_HTTP_RETRY_BACKOFF * attempt)
                continue
            return {"status": "error", "code": r.status_code, "detail": r.text[:200]}
        except httpx.TimeoutException:
            if attempt < _HTTP_MAX_RETRIES:
                await asyncio.sleep(_HTTP_RETRY_BACKOFF * attempt)
                continue
            return {"status": "timeout", "reason": f"POST timeout after {_HTTP_TIMEOUT}s", "url": url}
        except Exception as e:
            return {"status": "offline", "reason": str(e), "url": url}
...
async def _health_check() -> dict[str, Any]:
    """Vérifie que Titanium est accessible avant de démarrer le serveur MCP."""
    result = await _get("/api/state")
    if result.get("status") in ("offline", "timeout", "error"):
        return {"healthy": False, "detail": result}
    return {"healthy": True, "version": result.get("version", "unknown")}


# ── Imports dynamiques mis en cache ───────────────────────────────────────────

# Cache pour éviter les re-imports à chaque appel de tool
_import_cache: dict[str, Any] = {}


def _cached_import(module_path: str):
    """Importe un module une seule fois et le met en cache."""
    if module_path not in _import_cache:
        parts = module_path.split(".")
        mod = __import__(module_path, fromlist=[parts[-1]])
        _import_cache[module_path] = mod
...
async def read_resource(uri: str) -> list[ResourceContents]:
    if uri == "titanium://signals/latest":
        ...
        if data.get("status") in ("offline", "timeout", "error"):
            content = data
        else:
        ...
        if state.get("status") in ("offline", "timeout", "error"):
            content = state
        else:
...
async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    if name == "get_signal_history":
        ...
        if stats.get("status") in ("offline", "timeout", "error"):
            result = stats
        else:
            result = {
    ...
    # ── Phase 3 : Action tools (imports mis en cache) ───────────────────────
        ...
        try:
            # 🔒 Import mis en cache — exécuté une seule fois
            engine = _cached_import("engine.learning_engine")
            cfg = _cached_import("utils.config")
            SYMBOLS = getattr(cfg, "SYMBOLS", [])
            SCORE_CRITERIA = getattr(cfg, "SCORE_CRITERIA", [])
            ...
            if pair not in SYMBOLS:
                ...
                old = engine.scoring_weights[pair].get(criterion, 1.0)
                engine.scoring_weights[pair][criterion] = round(weight, 4)
                engine.save_state()
                result = {
                    "status": "ok",
                    "pair": pair,
        ...
        try:
            risk_mgr = _cached_import("execution.risk_manager")
            changes = {}
            risk_pct = arguments.get("risk_per_trade_pct")
            max_pos  = arguments.get("max_positions")

            if risk_pct is not None:
                old = getattr(risk_mgr, 'RISK_PCT', None)
...
async def _serve() -> None:
    ...
    if not health.get("healthy"):
        print(f"⚠️  [MCP] Titanium backend non accessible: {health}", file=sys.stderr)
        print("    Le serveur MCP démarre quand même, mais attendra des requêtes.", file=sys.stderr)
    else:
        print(f"✅ [MCP] Connecté à Titanium v{health.get('version', '?')}", file=sys.stderr)
